...
export default function PreviousRecordsScreen({ route }) {
  ...
  const handleSearch = () => {
    if (searchQuery === "") {
      ...
      const filteredResults = allRecords.filter(
        (record) => record.record_id === parseInt(searchQuery)
      );
  ...
  const handleSearchInputChange = (text) => {
    ...
    if (text === "") {
      ...
      const filteredResults = allRecords.filter(
        (record) => record.record_id === parseInt(text)
      );
      setSearchResults(filteredResults);
    }
  };
              ...
              <View key={record.record_id} style={styles.inputmultiline}>
                <View style={{ flexDirection: "column", marginTop: 5 }}>
                  ...
                  <View style={{ flexDirection: "row", marginTop: 3 }}>
                    <Text style={{ fontSize: 12, marginTop: 0, marginLeft: 0 }}>
                      Date : {record.savedDate}
                    </Text>
                    <Text
                      style={{ fontSize: 12, marginTop: 0, marginLeft: 50 }}
                    >
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate("MoniterFertilizationScreen", {
                        id: record.record_id,
                      })
                    }
                  >
            ...
            <View key={record.record_id} style={styles.inputmultiline}>
              <View style={{ flexDirection: "column", marginTop: 5 }}>
                ...
                <View style={{ flexDirection: "row", marginTop: 3 }}>
                  ...
                  <Text style={{ fontSize: 12, marginTop: 0, marginLeft: 50 }}>
                    Time : {record.savedTime}
                  </Text>
                </View>
                <TouchableOpacity
